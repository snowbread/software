...
另外远程桌面的客户端无法访问本地的远程桌面服务端，如果有这种需求的话，例如本地账户想在不切换用户的情况下访问本地的另一个账户，可以用中转的方法来实现，比如开个虚拟机，然后在虚拟机里面远程连接另一个账户，或者先远程另一台电脑，然后在另一台电脑上再远程本地电脑上的另一个账户。当然，这两种方法都比较麻烦，还有一个简单的办法，如果你的路由器能够做端口映射且你不在意把端口暴露到外网的话，那么可以将路由器的一个端口映射到本地电脑的远程桌面服务端口，这样在本地就可以远程连接路由器的那个端口，路由器把连接请求转发到本地电脑的远程桌面端口，这样就可以本地远程到本地的另一个账户了。  
