...
<h5>FAQ</h5>
<h6>修改并保存chm文件后再次打开还是没修改前的内容  或者 修改后关闭窗口不提示保存</h6>
这个软件本身存在bug, 在选择要修改的htm页面时, 不能通过左侧索引列表进入htm页面, 必须通过左侧的目录列表或文件列表进入htm页面(或者点击查找按钮并全局搜索进入也可),
否则修改htm页面后点击右上角关闭按钮不会提示你保存, 而是直接关闭程序, 导致修改内容丢失(即使你手动点击保存按钮再关闭程序也没有用, 虽然手动点击保存按钮会弹出保存窗口,
但实际上并没有保存). 
...
### qq自带的远程控制
这个只有双方电脑上都装有qq就可以了，不需要额外安装其它软件。
...
<h5>FAQ</h5>
<h6>如何取消虚拟桌面切换时任务栏的过渡动画</h6>
控制面板->系统->高级系统设置->高级->性能->视觉效果->取消勾选 任务栏和开始菜单中的动画 复选框
