...
最后一个支持 win7 的版本是 1.15.4，这个版本不支持指定传输所使用的网络  
  
默认开启了加密，这有可能会影响传输速度，如果传输速度慢的话尝试关闭加密(发送方和接收方都要关闭，关闭加密后把两边的软件都关闭重开一下)  
  
修改显示名称后如果对方看到的还是原来的名称，可以先把这边的软件关掉，然后让对方刷新一下，这时对方应该看不到这台机器了，然后再打开这边的软件，再让对方刷新一下，就应该显示新的名称了。  
